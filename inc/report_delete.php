...
try {
	...
	if($user->level<$config['permissions']['deleteRaport']) throw new Exception(NO_PERMISSION);

	$id = htmlentities($_POST['id'],ENT_QUOTES,'utf-8');

	$sql = "DELETE FROM `lspd_reports` WHERE `id`='$id'";
	if($db->query($sql)) {
		echo json_encode(['result'=>true, 'reason'=>'Poprawnie usunięto raport']);
